...
class LiveStreamManager:
    ...
    async def connect(self, ws: WebSocket) -> None:
        ...
        logger.info("browser_websocket_connected active_clients=%s", len(self.active_sockets))

    async def disconnect(self, ws: WebSocket) -> None:
        async with self._lock:
            if ws in self.active_sockets:
                self.active_sockets.remove(ws)
        logger.info("browser_websocket_disconnected active_clients=%s", len(self.active_sockets))
...
async def _build_operational_snapshot() -> dict[str, Any]:
    """Collect the slower broker, database, and strategy dashboard sections concurrently."""
    ...
    (
        ...
        asyncio.to_thread(broker_service.get_paper_positions),
        asyncio.to_thread(broker_service.get_paper_orders),
        asyncio.to_thread(strategy_service.get_signals_log, 25),
        asyncio.to_thread(strategy_service.get_strategies_stats),
        asyncio.to_thread(strategy_service.get_strategy_cards),
    )
    return {
        "positions": positions,
        "orders": orders,
        ...
        "detailed_strategies": details,
    }


async def _background_stream_loop() -> None:
    """Broadcast 1-second market ticks and a 5-second operational snapshot."""
    last_operational_refresh = 0.0
    while True:
        try:
            ...
            now = time.monotonic()
            if now - last_operational_refresh >= 5.0:
                operational = await _build_operational_snapshot()
                last_operational_refresh = now
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info("health_collector_started")
    except Exception as exc:  # noqa: BLE001
    ...
    try:
        price_feed = get_delta_price_feed()
        price_feed.start()
        logger.info("delta_price_feed_started")
    except Exception as exc:  # noqa: BLE001
        logger.warning("Could not start Delta price feed: %s", exc)

    private_stream = get_delta_websocket_client()
    try:
        broker_service = get_broker_service()
        private_stream.configure(
            broker_service.delta_client.api_key,
            broker_service.delta_client.api_secret,
        )
        private_stream.start()
    except Exception as exc:  # noqa: BLE001
        logger.warning("Could not start Delta private stream: %s", exc)
...
async def get_market_ticker_symbol(symbol: str) -> JSONResponse:
    ...
    if ticker is None:
        return JSONResponse(
            ...
            content={
                "status": "error",
                "message": f"No live data for {symbol} yet. Price feed may still be connecting.",
            },
        )
...
async def websocket_live_endpoint(websocket: WebSocket) -> None:
    ...
    try:
        ...
        operational, live_tickers = await asyncio.gather(
            _build_operational_snapshot(),
            asyncio.to_thread(_build_live_tickers_with_history),
        )
